...
const QuotationApp: React.FC = () => {
    ...
    const getAddressDetails = async (zipCode: string) => {
        try {
          ...
          if (data && data.length > 0) {
            ...
            setAddressDetails({ lat: parseFloat(lat), lng: parseFloat(lon), city, state });
    ...
    const calculateQuote = (distance: number, containerCost: number): {
      containerCost: number;
      deliveryCost: number;
      totalCost: number;
    } => {
    ...
    const handleSubmit = async () => {
      try {
        ...
        const quotes = Object.entries(Depots).map(([depotName, { lat, lng, containerCost }]) => {
          const distance = calculateDistance(details.lat, details.lng, lat, lng);
          const { containerCost: calculatedContainerCost, deliveryCost, totalCost } = calculateQuote(
            distance,
            containerCost
          );
      ...
      <div>
        ...
        {addressDetails && (
          <div>
            ...
            <p>Shipping to: {addressDetails.city}, {addressDetails.state} {zipCode}</p>
            ...
            <ul>
              {quotes.map((quote) => (
                <li key={quote.depot}>
                  ...
                  {quote.containerCost}, Shipping: ${quote.deliveryCost.toFixed(2)}, Total: ${quote.totalCost.toFixed(2)}
                </li>
              ))}
            </ul>
          </div>
